...
- **Local or authorized remote targets.** Works against a locally-run target *or* an arbitrary remote host you are authorized to test. Every request is scope-locked fail-closed to the declared target and hardened for remote use — cloud-metadata and link-local addresses are refused, a public name that resolves to a private/loopback IP is refused as DNS rebinding, the connection is pinned to the scope-validated IP (no connect-time re-resolution), redirects are re-validated per hop, and a challenge/rate-limit circuit-breaker aborts to `NOT DATA` rather than hammer the host. It is still **not** a mass-scanner: one authorized target, the identities you supply.
