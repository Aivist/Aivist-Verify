...
# Layer 3 — regenerate Layer 2 from Layer 1 with YOUR OWN Gemini key.
# 430 runs, but budget ~780 model calls: a run costs 1 call, or 2 when it takes a
# follow-up read-back. 350 of the 430 committed runs did. See REPRODUCE.md.
...
- **Authorized targets, remote or local.** Works against a locally-run target *or* a remote host you are authorized to test. Every request is scope-locked fail-closed to the declared target and hardened for remote use — cloud-metadata and link-local addresses are refused, a public name that resolves to a private/loopback IP is refused as DNS rebinding, an unresolvable target is refused, the connection is pinned to the scope-validated IP (no connect-time re-resolution), redirects are re-validated per hop, and a challenge/rate-limit circuit-breaker aborts to `NOT DATA` rather than hammer the host. These rails are **locked by committed tests** — the scope-lock + preflight refusals (SSRF / DNS-rebinding / link-local + metadata / unresolvable) in [`backend/tests/test_remote_safety_lock.py`](./backend/tests/test_remote_safety_lock.py), and, in [`backend/tests/test_remote_e2e_lock.py`](./backend/tests/test_remote_e2e_lock.py), a **real-socket non-loopback** scope-lock enforcement test plus the full `verify --target-file` CLI driven to a `CONFIRMED` over a non-loopback target (the engine is stubbed there for determinism). A run where a **real socket and a live model together** reach that `CONFIRMED` is archived verbatim at [`scripts/measure/real_targets/REMOTE_TARGET_RESULTS.md`](./scripts/measure/real_targets/REMOTE_TARGET_RESULTS.md) — an engineering signal, not a benchmark entry. It is still **not** a mass-scanner: one authorized target, the identities you supply.
- **No authentication of its own.** This is about the **tool process**, not the target: Aivist Verify has no access control around *itself*, so run the confirmer where only you can reach it and never expose *it* as a network service. (The **target** may be a local or an authorized remote host — see above.) You supply no auth of its own; it acts only as the identities whose tokens you give it.
...
**Supported (built and audited in-repo):** local **and authorized-remote** target confirmation (scope-lock + SSRF / DNS-rebinding guard + resolved-IP pinning + challenge circuit-breaker); **path *and* query-string (non-path) object ids** — an id in `?report_id=` is expressed, attacked, and owner-view-corroborated exactly like an id in the path; OpenAPI-spec and spec-less discovery (manual endpoint lists, HAR / raw-HTTP parsing, live mitmproxy capture); static-token *and* automatic re-login auth; three model providers behind one seam (Gemini default, OpenAI-compatible incl. **DeepSeek** / relays / local, Anthropic); the fully non-interactive `run --config` entry for CI; a packaged **GitHub Action** that turns that entry into an access-control regression gate (confirm-only, fail-on-confirm); an explicit **tiered-verdict framework** (`CONFIRMED` for deterministic proof · `SIGNAL` for evidence-inferred leads · `REFUTED` · `NOT DATA`) with `CONFIRMED` reserved *by construction* for deterministic detectors; **SSRF confirmation via out-of-band callback** (`aivist ssrf`) — the **first non-access-control vuln type**, `CONFIRMED` only when the target makes a real [interactsh](https://github.com/projectdiscovery/interactsh) callback to a unique probe domain (see [`docs/SSRF.md`](./docs/SSRF.md)); and **blind SQL-injection confirmation via out-of-band callback** (`aivist sqli`) — a further type, `CONFIRMED` only when the target's DBMS makes a **token-matched, injection-causal** interactsh callback (the callback's token corresponds one-to-one to a payload we actually injected), with error-based / time-based blind leads capped at `SIGNAL` (see [`docs/SQLI.md`](./docs/SQLI.md)).

**Limits, stated plainly:** the **statistical** zero-false-positive record comes from **two controlled labs**; the engine has additionally been validated against **two public real targets** (crAPI and VAmPI — nine hand-verified runs, [above](#validated-on-real-public-targets--not-just-our-own-labs)), but it has **not** been run at scale across diverse production systems — "supported" means the capability exists and is audited, not that it's been battle-tested in the wild. Remote support means the **networking + scope safety** is built and **locked by committed tests** (`backend/tests/test_remote_safety_lock.py` + `backend/tests/test_remote_e2e_lock.py`); it does not add a new class of finding. The read-semantic confirmation gate has **documented bounds** (see `RESULTS.md`), including a deliberate miss on broken-for-all resources. The tiered-verdict framework is **in place**, and two detectors are wired to it: **access control** reaches `CONFIRMED` via its deterministic code gate, and **SSRF** reaches `CONFIRMED` via a real out-of-band callback (the interactsh interaction is the deterministic proof). SSRF is the first non-access-control type; the `SIGNAL` tier remains a reserved extension point for a future inference-only detector. **SSRF confirmation needs outbound network and a reachable interactsh server** — the **out-of-band (interactsh) client** (`services/oob/`, [`docs/OOB.md`](./docs/OOB.md)) is now **consumed by the SSRF detector** (`services/ssrf_detector.py`, [`docs/SSRF.md`](./docs/SSRF.md)); with no reachable server it returns `NOT DATA`, never a guess, and a target that makes no callback is `REFUTED`, never a bluffed `CONFIRMED`. **Model independence:** zero-FP is a property of the deterministic **code gate**, not the model — swapping in DeepSeek (`LLM_PROVIDER=openai`, `deepseek-chat`) is *expected* to confirm the REAL cases and refute the SAFE ones exactly as Gemini does, and it did so in a live session — but that equivalence is **not yet locked by a committed test**, and a non-Gemini backend gets connectivity, not a re-measured zero-FP guarantee (see [`docs/LLM_PROVIDERS.md`](./docs/LLM_PROVIDERS.md)). The tool still has **no authentication of its own** — run it where only you can reach it.
...
│   └─ services/oob/         # interactsh OOB client — now consumed by the SSRF detector
│   └─ services/ssrf_detector.py  # SSRF confirmation via OOB (first non-access-control type)
...
├─ ssrf_target/              # lab 4 (SSRF: REAL /fetch + SAFE /fetch-safe) + ground-truth suite
