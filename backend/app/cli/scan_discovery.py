...
_DISCOVERY_SYSTEM_PROMPT = (
    ...
    "IMPORTANT - what counts as an object id: ANY {templated} path segment is a candidate object id, "
    "WHATEVER it is named or shaped. It does NOT have to be numeric or end in '_id'. A name, slug, "
    "title, username, uuid, or hash id all qualify - e.g. {book_title}, {slug}, {username}, {uuid}, "
    "{isbn} are EACH object ids just like {order_id} or {id}. Treat EVERY {templated} path segment as a "
    "possible object id and propose it as a candidate; do not skip an endpoint because its id segment is "
    ...
    "  id_param      - for path: the template variable name EXACTLY as written between the braces\n"
    "                  (e.g. \"order_id\" for /orders/{order_id}; \"book_title\" for /books/{book_title});\n"
