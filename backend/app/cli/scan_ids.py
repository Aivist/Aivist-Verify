...
async def source_ids(
    ...
    Tier a (id map) wins when present. Tier b harvests the declared collection AS THE ATTACKER
    (attacker creds only) and AS THE OWNER (owner creds only) on SEPARATE clients; the attacker id is
    the attacker's first own id, the victim id is the owner's first id the attacker does NOT own (a
    genuinely cross-user target). Never fabricates: no sourceable pair -> None."""
    ...
    # Tier b — declared collection endpoint, harvested per-account (identity-isolated).
    coll = (collections or {}).get(key)
    if coll and attacker_cred is not None and owner_cred is not None:
        id_param = candidate.get("id_param", "id")
        atk_ids = await _harvest(coll, id_param, base_url=base_url, approved_host=approved_host,
                                 cred=attacker_cred, control_view=control_view,
                                 client_factory=client_factory)          # ATTACKER creds ONLY
        own_ids = await _harvest(coll, id_param, base_url=base_url, approved_host=approved_host,
