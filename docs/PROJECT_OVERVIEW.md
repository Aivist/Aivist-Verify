...
> ✅ **D24 已修复:五种形态现在全部由代码闸门守住(`033fc9e`)。** 此处原本记录的问题是:上述 140 次 SAFE 中,**有 20 次(读型语义等价 X-EQUIV-SAFE)背后并没有任何代码闸门**——`guard_override` 全程为 `None`,最终判定完全等于模型原始判定;该形态的「零误报」靠的是**模型恰好答对**,而不是代码守住,并且在第二个靶场 `depot_target/` 上同一形态**确定性地误报 20/20**。
