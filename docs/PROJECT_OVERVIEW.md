...
| 测试 | **后端 647 个自动化测试全绿**(pytest)+ 靶机 31 + depot_target 23 个(独立套件),覆盖核心服务 + API 层 + 被动代理雷达 + 验证判定层(含 B-1 影子路径回归测试 + M1.1/M1.2 判定与豁免测试 + D21 spec-config 测试 + scope-lock 对抗/一致性测试) |
...
> **当前权威度量为「黄金」双靶高 N 记录**:五种形态 × **两个结构不同的靶场**(`vulnerable_target` 整数 id、`depot_target` UUID id)× 真实 gemini-2.5-pro,SAFE/控制组 N=20、VULN N=10,每次全新播种:**300 次 SAFE/控制组 → 0 误报;130 次 VULN → 全部 `verified`(各走其预期通道);430/430 全部可用、0 降级**(`scripts/measure/results/sweep_highN.jsonl`,可复现见 `scripts/measure/REPRODUCE.md`)。**头条证据:** 所有 SAFE 用例合计,模型**原始判定想放行 79 次,代码逐一拒绝、无一漏网**(`DP-READ-SAFE`/`-ECHO` 各 20/20 走 D24 owner-view 闸门;`X-MASS-SAFE` ×2 走 state-jump;`X-EQUIV-SAFE` **1/20**——模型在 1 号靶的读型上翻成 `verified` 一次,**若无 D24 闸门这一次即为误报**)。此记录**取代**早先的单靶 140/70 记录(作为历史保留于 `RESULTS.md`)。下文各形态的 `5/5` 是最初的 N=5 单形态记录。
> ✅ **D24 已修复:五种形态现在全部由代码闸门守住(`033fc9e`)。** 此处原本记录的问题是:在早先的单靶 140 次 SAFE 中,**有 20 次(读型语义等价 X-EQUIV-SAFE)背后并没有任何代码闸门**——`guard_override` 全程为 `None`,最终判定完全等于模型原始判定;该形态的「零误报」靠的是**模型恰好答对**,而不是代码守住,并且在第二个靶场 `depot_target/` 上同一形态**确定性地误报 20/20**。
> **修复方式:owner-view 差分闸门**——读型是唯一「代码不采集任何证据」的形态,现在由代码**以属主(owner)身份**读取同一对象(依赖双账号凭据通道 `5a33cb2`),只有当攻击响应**印证属主的真实视图**时 `verified` 才成立;不依赖任何「拒绝关键词」。闸门**结构上只能降级**,且仅作用于「无 follow-up」分支,其余四种形态代码路径一行未改。
> **已用真实模型复验**(N=1 × 5 个读型用例):`DP-READ-SAFE` 从 `verified` 变为 **`inconclusive`**(`owner_view_not_corroborated`),而模型原始判定**仍然是 `verified`**——守住底线的是代码。
> ⚠️ **但不等于该形态已彻底解决**,四条边界仍然成立(见 [`TECH_DEBT.md`](./TECH_DEBT.md) D24):0.95 阈值是在**确定性实验室数据、原始响应体**上标定的,**尚未针对真实目标的高熵字段(时间戳/ETag)验证**(复用现有 `_sanitize_response_text` 的方案已实测**否决**——它反而把 SECURE 的相似度抬高);**公共/共享资源**仍是降级式闸门无法覆盖的残留缺口;属主凭据是**每部署一份,而非每 finding 一份**;真实模型复验只有 **N=1,未做规模化**。
...
- **自动化测试:后端 647 个,全部通过**(本会话亲测;`python -m pytest backend/tests -q`)。另有靶机独立套件 31 个(`vulnerable_target`)+ 23 个(`depot_target`)。覆盖:
...
#   关键项：GEMINI_API_KEY、API_HOST(默认 127.0.0.1)
...
### 验收清单（建议逐项打勾）
- [ ] **测试全绿**:仓库根目录运行 `python -m pytest backend/tests -q` → 应见 `647 passed`。
